...
module.exports = async function(config, slug, app) {

  var sheetOpsFactory = require("./sheetOps"),
      sheetOps = sheetOpsFactory(app);
  ...
  var handlers = {
    ...
    ".js": async function({ full, relative }) {
      var { src, map } = await processJS(full, { production: true, root: config.root });
  ...
  var previewData = { embedPath, copyeditPath, directLinkPath, COPY, slug, config, sheet, stylesheet, deployed: true };
